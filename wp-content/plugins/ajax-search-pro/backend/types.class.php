...
if (!class_exists("wpdreamsCustomFields")) {
  class wpdreamsCustomFields extends wpdreamsType {
      ...
      global $wpdb;
      $this->processData();
      $this->types = $wpdb->get_results("SELECT * FROM ".$wpdb->postmeta." WHERE substring(meta_key,1,1)<>'_' GROUP BY meta_key", ARRAY_A);
  		echo "
      <div class='wpdreamsCustomFields'>
        <fieldset>
          <legend>" . $this->label . "</legend>";
      echo '<div class="sortablecontainer">Available public custom fields types<ul id="sortable'.self::$_instancenumber.'" class="connectedSortable">';
      ...
      echo "</ul></div>";
      echo '<div class="sortablecontainer">Drag here the custom fields you want to use!<ul id="sortable_conn'.self::$_instancenumber.'" class="connectedSortable">';
...
add_action('admin_print_styles', 'admin_stylesV02');
add_action('admin_enqueue_scripts', 'admin_scriptsV02');
...
function admin_scriptsV02() {
  ...
  wp_register_script('wpdreams-jqPlot', plugin_dir_url(__FILE__) . '/types/js/jqPlot/jquery.jqplot.min.js', array(
		'jquery'
	));
  wp_enqueue_script('wpdreams-jqPlot');
  wp_register_script('wpdreams-jqPlotdateAxisRenderer', plugin_dir_url(__FILE__) . '/types/js/jqPlot/plugins/jqplot.dateAxisRenderer.min.js', array(
		'wpdreams-jqPlot'
	));
  wp_enqueue_script('wpdreams-jqPlotdateAxisRenderer');
  wp_register_script('wpdreams-jqPlotcanvasTextRenderer', plugin_dir_url(__FILE__) . '/types/js/jqPlot/plugins/jqplot.canvasTextRenderer.min.js', array(
		'wpdreams-jqPlot'
	));
  wp_enqueue_script('wpdreams-jqPlotcanvasTextRenderer');  
  wp_register_script('wpdreams-jqPlotcanvasAxisTickRenderer', plugin_dir_url(__FILE__) . '/types/js/jqPlot/plugins/jqplot.canvasAxisTickRenderer.min.js', array(
		'wpdreams-jqPlot'
	));
  wp_enqueue_script('wpdreams-jqPlotcanvasAxisTickRenderer'); 
  wp_register_script('wpdreams-jqPlotcategoryAxisRenderer', plugin_dir_url(__FILE__) . '/types/js/jqPlot/plugins/jqplot.categoryAxisRenderer.min.js', array(
		'wpdreams-jqPlot'
	));
  wp_enqueue_script('wpdreams-jqPlotcategoryAxisRenderer');
  wp_register_script('wpdreams-jqPlotbarRenderer', plugin_dir_url(__FILE__) . '/types/js/jqPlot/plugins/jqplot.barRenderer.min.js', array(
		'wpdreams-jqPlot'
	));
  wp_enqueue_script('wpdreams-jqPlotbarRenderer');      
  
}
function admin_stylesV02() {
  ...
  wp_enqueue_style('wpdreams-jqueryui');
	...
	wp_register_style('wpdreams-jqPlotstyle', plugin_dir_url(__FILE__) . '/types/js/jqPlot/jquery.jqplot.min.css');
